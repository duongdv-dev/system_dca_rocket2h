...
class PresetOptimizer:
    ...
    def backtest_out_of_sample(self, 
                                test_features_df: pd.DataFrame, 
                                test_days_data: Dict[str, pd.DataFrame],
                                train_best_presets: Dict[str, Any],
                                kmeans_model: KMeans,
                                scaler_model: StandardScaler) -> Dict[str, Any]:
        """
        Kiểm định độc lập trên dữ liệu Out-of-Sample (Tập Test 2024-2025).
        """
        feature_cols = ['asian_range_atr_ratio', 'asian_atr_m15', 'asian_return_pct', 'asian_body_ratio']
        ...
        df_test = test_features_df.copy()
        df_test['regime_id'] = test_labels

        test_summary = {}

        for regime_key, train_info in train_best_presets.items():
            r_id = int(regime_key.replace('Regime_', ''))
            preset = train_info['preset']

            if preset.get('entry_rule') == 'NO_TRADE':
                test_summary[regime_key] = {'total_profit_usd': 0.0, 'profit_factor': 0.0, 'win_rate': 0.0, 'max_drawdown_pct': 0.0}
                continue

            regime_test_days = df_test[df_test['regime_id'] == r_id]
            daily_results = []
            cum_pnl = 0.0
            peak_pnl = 0.0
            max_dd_usd = 0.0

            for _, row in regime_test_days.iterrows():
                date_str = row['date']
